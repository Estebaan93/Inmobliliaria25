...
namespace Inmobiliaria25.Repositorios
{
  public class RepositorioInmueble
  {
    ...
    public Inmueble? Obtener(int idInmueble)
    {
      ...
      {
        conn.Open();
        string sql = @"SELECT i.idInmueble, i.idPropietario, i.idTipo, i.descripcion, 
                              i.cantidadAmbientes, i.precio, i.cochera, i.piscina, i.mascotas, i.estado, i.metros2, i.UrlImagen,
        ...
        using (var cmd = new MySqlCommand(sql, conn))
        {
          ...
          using (var reader = cmd.ExecuteReader())
          {
            if (reader.Read())
            {
              ...
              // aca mapeamos la direccion
              i.Direccion = new Direccion
              {
